...
def mean_artist_play(time,writer):
    with open(ARTIST_P_D_C, "r") as fr:
        ...
        while artist_id:
            ...
            download = list(map(int, fr.readline().strip("\n").split(",")))
            collect = list(map(int, fr.readline().strip("\n").split(",")))
            play1=np.array(play[174:183])  #9天
            #play1=np.array(play[176:183])
            play2=np.array(play[168:183])   #15天
            ...
            myplay =list( np.array(play[169:183]))   #14天
            #myplay =list(play1)
            if sigma1 <= sigma2:
                min1 = min(play1)
                max1 = max(play1)
                sum1 = (sum(play1) - min1 - max1) / 7
                myplay.append(sum1*(1+col_in_download))
            else:
                min2 = min(play2)
                max2 = max(play2)
                sum2 = (sum(play2) - min2 - max2) / 13
                myplay.append(sum2*(1+col_in_download))
